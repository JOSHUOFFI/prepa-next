...
export function QuestionBank() {
    ...
    useEffect(() => {
        ...
        setPage(1);
    }, [filters.search, filters.subject, filters.class, filters.term, filters.topic, filters.difficulty, filters.type, filters.status]);

    useEffect(() => {
        const params = new URLSearchParams({ page: String(page), pageSize: String(pageSize), status: filters.status });
        ...
        (async () => {
            ...
            if (!response.ok) { setError(payload.error ?? "Unable to load questions."); } else { setQuestions(payload.questions ?? []); setTotal(payload.total ?? 0); }
        })();
    }, [filters, page, pageSize]);

    async function setActive(id: string, active: boolean) {
        await fetch(`/api/admin/questions/${id}`, { method: active ? "PUT" : "DELETE" });
        setPage(1);
    }
    function load(targetPage: number = 1) {
        setPage(targetPage);
    }
